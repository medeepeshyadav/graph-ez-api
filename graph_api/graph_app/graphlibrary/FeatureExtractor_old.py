...
class FeatureExtractor:
    ...
    def _get_features(self, X):
        ...
        if basic:
            ...
            for feature in features1:
                ...
                # else:
                #     tqdm.pandas(desc=f"creating {feature} column")
                #     X[feature] = X.progress_apply(lambda row: func(row['destination']), axis=1)
        ...
        if advanced:
            ...
            #Page Rank
            # X['source_pagerank'] = X['source'].map(lambda x: pr[int(x)])
            # X['destination_pagerank'] = X['destination'].map(lambda x: pr[int(x)])

            #Katz Centrality
            mean_katz = float(sum(katz.values())) / len(katz)

            for feat in ['katz_source', 
                        'katz_destination',
                        'hits_source',
                        'hits_destination',
            ...
            # weight_list = self.weighted_features()
            # tqdm.pandas(desc='creating param_dependent_index_for_successors column')
            # X['weight_source'] = X['source'].progress_apply(lambda x: weight_list[0].get(x,weight_list[2]))

            # tqdm.pandas(desc='creating param_dependent_index_for_successors column')
            # X['weight_destination'] = X['destination'].progress_apply(lambda x: weight_list[1].get(x,weight_list[3]))
            ...
            # X['destination'].progress_apply(lambda x: self.svd(x, U, sadj_dict)).apply(pd.Series)
            # #===================================================================================================
    ...
    # def _findSuccessors(self, nodelist1: pd.Series, nodelist2: pd.Series):
    #     allNodes = set(nodelist1).union(set(nodelist2))
    #     succ_dict = {}
    #     pred_dict = {}
    #     for node in allNodes:
    #         try:
    #             succ_dict[node] = set(self.graph.successors(node))
    #             pred_dict[node] = set(self.graph.predecessors(node))

    #         except:
    #             continue

    #     return succ_dict, pred_dict
    ...
    def jaccard(
        ...
        try:
            if for_what == 'successors':
                ...
                # x_ = self.predecessors_dict[x]
                # y_ = self.predecessors_dict[y]
